...
# testing if PyOpenGL has been installed
try: # PyOpenGL
...
# testing if GLFW has been installed
try: # GLFW
  import glfw
except:
...
def buildHull( points ):
    ...
    #3 points
        ...
        #right turn
        if (turn(points[0], points[1], points[2]) == 2):
            points[0].cwPoint = points[1]
            ...
            #left turn
        elif (turn(points[0], points[1], points[2]) == 1):
    ...
    # Handle recursive case.
        ...
        arrayLeft = points[:(int(len(points)/2))]
        buildHull(arrayLeft)
        arrayRight = points[(int(len(points)/2)):]
        buildHull(arrayRight)
        deleteArray = []

    # find CW recursive function
    # find CCW recursive function
    ...
    # from interior points disappear after you do this.
        ...
        if ((len(arrayLeft) + len(arrayRight)) > 3):
            deleteArray.append(left)
            deleteArray.append(right)

        #walking up
        while (turn(left.ccwPoint, left, right) == 1 or turn(left, right, right.cwPoint) == 1):
            if (turn(left.ccwPoint, left, right) == 1):
                left = left.ccwPoint
                if ((len(arrayLeft) + len(arrayRight)) > 3):
                    deleteArray.append(left)
            else:
        ...
        if (left in deleteArray):
            deleteArray.remove(left)

        if (right in deleteArray):
            deleteArray.remove(right)

        for i in range(0, len(arrayRight)):
            if arrayRight[i].x == right.x and arrayRight[i].y == right.y:
                rightIndexTop = i
        for i in range(0, len(arrayLeft)):
            if arrayLeft[i].x == left.x and arrayLeft[i].y == left.y:
                leftIndexTop = i
        ...
        #walking down
        while (turn(left.cwPoint, left, right) != 1 or turn(right.ccwPoint, right, left) != 2):
            if (turn(left.cwPoint, left, right) != 1):
                left = left.cwPoint
                if ((len(arrayLeft) + len(arrayRight)) > 3):
                    deleteArray.append(left)
            else:
        ...
        arrayRight[rightIndexTop].ccwPoint = arrayLeft[leftIndexTop]
        arrayLeft[leftIndexTop].cwPoint = arrayRight[rightIndexTop]

        arrayRight[rightIndexBottom].cwPoint = arrayLeft[leftIndexBottom]
        arrayLeft[leftIndexBottom].ccwPoint = arrayRight[rightIndexBottom]

        for i in range(0, len(deleteArray)):
            deleteArray[i].cwPoint = None
            deleteArray[i].ccwPoint = None
